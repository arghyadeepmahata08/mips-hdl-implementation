`timescale 1ns/1ns      //Timescale Definition
...
localparam OP_J = 6'b000010, //Jump
        ...
        OP_R_TYPE = 6'b000000, //Most r-type instructions have same opcode
        OP_MFC0 = 6'b010000, //Move from C0
        OP_BLTZ = 6'b000001, //Branch on less than zero
        OP_BEQ = 6'b000100, //Branch on equal
        OP_BNE = 6'b000101, //Branch on not equal
        OP_BLEZ = 6'b000110, //Branch on less than or equal to zero
...
always @(*) begin
    //Initialise all control signals to zero
    PC_EN        = 1'b0;
    IorD         = 1'b0;
    MEM_OE       = 1'b0;
    MEM_WS       = 1'b0;
    ...
    case (Present_State)
        0: begin //State 0
            ...
            //Initialise all other outputs to zero
            MEM_WS = 1'b0;
            REG_WS = 1'b0;
            EPC_EN = 1'b0;
            CAUSE_EN = 1'b0;
        ...
        2: begin
            if (Opcode == OP_MFC0) begin
                if (Rd == 5'b01101) begin //State 33
                    ...
                    EPC_EN = 1'b1; //enable writing the current PC to the EPC register
                ...
                PC_SEL = 3'b010; //select (PC[31:28] || Instruct[25:0] || 00) as the next PC
                ...
                ALU_SEL1 = 1'b1; //select Reg1 as the first ALU operand
                ALU_SEL2 = 3'b010; //select immediate value (sign extend I([15:0]) as the second ALU operand
            ...
            else if (Opcode == OP_ADDi || Opcode == OP_LW || Opcode == OP_LB || Opcode == OP_LBu || Opcode == OP_LHu || Opcode == OP_LH || Opcode == OP_SB || Opcode == OP_SH|| Opcode == OP_SW) begin
            ...
            else begin //go to invalid state 31
                CAUSE_SEL = 1'b1; //select '1' to be written to the Cause register indicating an invalid operation
                CAUSE_EN = 1'b1; //enable writing to the Cause register
                EPC_EN = 1'b1; //enable writing the current PC to the EPC register
                PC_SEL = 3'b100; //select the incremented PC to be the next PC value
        ...
        3: begin
            if ((Opcode == OP_ORi) || (Opcode == OP_XORi) || (Opcode == OP_ANDi) || (Opcode == OP_SLTi) || (Opcode == OP_ADDiu) || (Opcode == OP_SLTiu) || ((Opcode == OP_MFC0) && (Rd == 5'b01110)) || ((Opcode == OP_MFC0) && (Rd == 5'b01101))) begin //State 8
                ...
                PC_SEL = 3'b010; //select content of (PC[31:28] || Inst[25:0] || 00) as the next PC
                PC_EN = 1'b1; //enable write to the PC register
                REG_WS = 1'b1; //enable writing to the Register File
                Next_State = 3'b000; //next state is FETCH
